...
class SudokuUI(Frame):
    ...
    def __start_solver(self):
        ...
        for i in range(self.problem.problem_num):
            for m in range(1, 10):
                for n in range(1, 10):
                    if self.problem.given_number[m - 1][n - 1] != 0:
                        self.canvas.itemconfig(9 * (m - 1) + n, text=self.problem.given_number[m - 1][n - 1],
            ...
            if i != self.problem.problem_num - 1:  self.problem = Problem(self.canvas, self.problem.tk, 0.0,
                                                                          self.problem.temp,
                                                                          setting=self.problem.setting)
        ...
        for i in range(0, 9):
            for j in range(0, 9):
                if self.given_number[i][j] != 0:
                    new.append(self.given_number[i][j])
                else:
                    new.append(0)
        ...
        for i in range(1, 10):
            for j in range(1, 10):
                if given_number[i - 1][j - 1] == 0:
                    domain = self.calc_constraints(given_number, i, j)
